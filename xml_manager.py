...
def parseFrameTrans(frame_node):
    ...
    sensor_rotation = (sensor_rx, sensor_ry, sensor_rz, sensor_rw)
    ...
    patient_sensor_pos = (sensor_x, sensor_y, sensor_z)
    ...
    patient_sensor_rotation = (sensor_rx, sensor_ry, sensor_rz, sensor_rw)
...
def parseInitPatientSensor(init_patient_node):
    ...
    sensor_y = patient_sensor_location_node.getElementsByTagName('Y')[0].firstChild.nodeValue
    sensor_z = patient_sensor_location_node.getElementsByTagName('Z')[0].firstChild.nodeValue
    init_patient_pos = (sensor_x, sensor_y, sensor_z)
    ...
    sensor_rz = patient_sensor_orien_node.getElementsByTagName('Z')[0].firstChild.nodeValue
    sensor_rw = patient_sensor_orien_node.getElementsByTagName('W')[0].firstChild.nodeValue
    init_patient_orien = (sensor_rx, sensor_ry, sensor_rz, sensor_rw)
...
def parseCalibrationDepth(depth_node):
    ...
    return {
        ...
        'origin_x': origin_x,
        'origin_y': origin_y,
        'x_millmeter_per_pixel': x_millmeter_per_pixel,
        'y_millmeter_per_pixel': y_millmeter_per_pixel
    }
...
def parseCalibrationTransform(calibration_node):
    ...
    for i in range(1, 4):
        for j in range(1, 4):
            ...
            matrix_elements.append(elmt.firstChild.nodeValue)
    ...
    for tag_name in ('X', 'Y', 'Z'):
        ...
        vector_elements.append(elmt.firstChild.nodeValue)
...
def parseXml(xml_path):
    ...
    for scan_node in root_node.getElementsByTagName('StudyFrameIndices'):
        ...
        scan_dict['es_patient_locationn'] = patient_sensor_pos
...
if __name__ == '__main__':
    ...
    xml_path = '/staff/wangzhaohui/ultrasound/AutoPointing/test/SID_3010_10020/VpStudy.xml'
    reader = XmlReader(xml_path)
    info = reader.read_scan_info('Scan5')
    print(info)